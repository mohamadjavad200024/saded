# آپدیت فوری server.js در هاست

## مشکل:
- server.js در هاست آپدیت نشده
...
# 1. آپدیت server.js با logging کامل
...
# 5. بررسی logs جدید
echo "=== Output logs (last 50 lines) ==="
...
tail -200 /home/shop1111/public_html/saded/logs/pm2-out-0.log | grep -E "ERROR|Error|error|Exception|Rejection|CAUGHT|WARNING" | tail -30
```

## نکات:

1. **اگر `[DEBUG] Next.js app prepared successfully` دیده شد:** server.js آپدیت شده ✅
2. **اگر `[requestId] ========== ERROR CAUGHT ==========` دیده شد:** خطا در catch block است
3. **اگر خطا دیده نشد اما 500 می‌دهد:** مشکل از Next.js internal است - باید error logs را بررسی کنیم
