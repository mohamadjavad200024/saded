module.exports=[30056,e=>e.a(async(t,a)=>{try{let t=await e.y("pg");e.n(t),a()}catch(e){a(e)}},!0),19152,e=>e.a(async(t,a)=>{try{var s=e.i(30056),r=e.i(40911),E=t([s]);[s]=E.then?(await E)():E,process.env.DB_PASSWORD||(r.logger.error("⚠️  WARNING: DB_PASSWORD is not set in environment variables!"),r.logger.error("   Please make sure .env file exists and contains DB_PASSWORD"),r.logger.error("   If you just updated .env, restart your Next.js server"),r.logger.error("   Current working directory:",process.cwd()));let R={host:process.env.DB_HOST||"localhost",port:parseInt(process.env.DB_PORT||"5432"),database:process.env.DB_NAME||"saded",user:process.env.DB_USER||"postgres",password:process.env.DB_PASSWORD||"",max:20,idleTimeoutMillis:3e4,connectionTimeoutMillis:2e3},O=null;function T(){return O||(O=new s.Pool(R)).on("error",e=>{r.logger.error("Unexpected error on idle PostgreSQL client",e)}),O}async function A(){try{let e=T();return(await e.query("SELECT NOW()")).rows.length>0}catch(e){return r.logger.error("Database connection test failed:",e),!1}}async function i(e,t){let a=await T().connect();try{return await a.query(e,t)}catch(t){throw r.logger.error("❌ Database query error:",{message:t?.message,code:t?.code,query:e.substring(0,100),hasPassword:!!R.password}),t}finally{a.release()}}async function N(e,t){return(await i(e,t)).rows[0]||null}async function n(e,t){return(await i(e,t)).rows}async function d(e){let t=await T().connect();try{await t.query("BEGIN");let a=await e(t);return await t.query("COMMIT"),a}catch(e){throw await t.query("ROLLBACK"),e}finally{t.release()}}async function o(){O&&(await O.end(),O=null)}async function c(){let e=`
  ...
  `;try{await i(t),r.logger.debug("✅ Database tables initialized successfully")}catch(e){throw r.logger.error("❌ Error initializing database tables:",e),e}}async function I(){let e=new s.Pool({...R,database:"postgres"});try{let t=await e.query("SELECT 1 FROM pg_database WHERE datname = $1",[R.database]);0===t.rows.length?(await e.query(`CREATE DATABASE ${R.database}`),r.logger.debug(`✅ Database '${R.database}' created successfully`)):r.logger.debug(`✅ Database '${R.database}' already exists`)}catch(e){if("42P04"!==e.code)throw r.logger.error("Error ensuring database:",e),e}finally{await e.end()}}e.s(["closePool",()=>o,"ensureDatabase",()=>I,"getPool",()=>T,"initializeTables",()=>c,"query",()=>i,"queryAll",()=>n,"queryOne",()=>N,"testConnection",()=>A,"transaction",()=>d]),a()}catch(e){a(e)}},!1)];
