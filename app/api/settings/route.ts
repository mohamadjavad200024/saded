...
export async function GET(request: NextRequest) {
  // Immediate response to test if route is being called
  console.log("=== [GET /api/settings] ROUTE HANDLER CALLED ===");
  console.log("Request URL:", request.url);
  console.log("Request method:", request.method);
  
  try {
    logger.info("[GET /api/settings] Starting request");
    console.log("[GET /api/settings] Starting request - DEBUG");
    
    // Ensure settings table exists
    logger.info("[GET /api/settings] Creating table if not exists");
    console.log("[GET /api/settings] Creating table if not exists - DEBUG");
    try {
      await runQuery(`
      ...
      `);
      logger.info("[GET /api/settings] Table ensured");
      console.log("[GET /api/settings] Table ensured - DEBUG");
    } catch (dbError: any) {
      logger.error("[GET /api/settings] Database error creating table:", dbError);
      console.error("[GET /api/settings] Database error creating table:", dbError);
      throw dbError;
    }

    // Get settings from database
    logger.info("[GET /api/settings] Fetching settings from database");
    console.log("[GET /api/settings] Fetching settings from database - DEBUG");
    ...
    logger.error("[GET /api/settings] Error getting settings:", error);
    console.error("[GET /api/settings] Error getting settings:", error);
    logger.error("[GET /api/settings] Error details:", {
      message: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : undefined,
      name: error instanceof Error ? error.name : undefined,
    });
    console.error("[GET /api/settings] Error details:", {
      message: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : undefined,
      name: error instanceof Error ? error.name : undefined,
    });
    
    // Return a simple 500 error instead of letting createErrorResponse convert it
    if (error instanceof AppError && error.status === 403) {
      // If it's already a 403, return it as is
      return createErrorResponse(error);
    }
    
    // For other errors, return 500 instead of 403
