# بررسی و آپدیت server.js

## مشکل:
- لاگ‌های جدید دیده نمی‌شوند
...
# 1. بررسی محتوای server.js فعلی
echo "=== Current server.js (first 20 lines) ==="
head -20 server.js

# 2. بررسی اینکه آیا [DEBUG] در فایل وجود دارد
echo "=== Checking for [DEBUG] in server.js ==="
grep -c "\[DEBUG\]" server.js && echo "✓ [DEBUG] found" || echo "✗ [DEBUG] NOT found - file needs update"

# 3. اگر [DEBUG] پیدا نشد، آپدیت server.js
...
EOFMARKER
  ...
  echo "✓ server.js already has [DEBUG] - no update needed"
fi
...
# 6. بررسی logs برای [DEBUG]
echo "=== Checking for [DEBUG] in logs ==="
...
## اگر بعد از آپدیت هنوز خطا دیده نمی‌شود:

مشکل از Next.js internal error handling است. باید:
1. بررسی کنیم که آیا `.next` folder کامل است
2. بررسی کنیم که آیا database connection مشکل دارد
3. بررسی کنیم که آیا import کردن module مشکل دارد
